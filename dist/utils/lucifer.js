...
exports.encryptLucifer = encryptLucifer;
exports.decryptLucifer = decryptLucifer;
exports.encryptLuciferBlocks = encryptLuciferBlocks;
exports.decryptLuciferBlocks = decryptLuciferBlocks;
function feistelRound(left, right, keyPart) {
    const f = (right ^ keyPart) >>> 0;
    const newLeft = right;
    const newRight = (left ^ f) >>> 0;
    return [newLeft, newRight];
}
function generateSubKeys(key, rounds = 16) {
    const subKeys = [];
...
function encryptLucifer(text, key) {
...
function decryptLucifer(cipherHex, key) {
...
function encryptLuciferBlocks(text, key) {
    const blocks = [];
...
function decryptLuciferBlocks(cipher, key) {
    const blocks = [];
...
//# sourceMappingURL=lucifer.js.map