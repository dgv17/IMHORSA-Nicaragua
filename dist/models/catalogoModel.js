...
async function getDepartamentos() {
    const [rows] = await db_1.pool.query("SELECT id, nombre FROM departamentos");
    return rows;
}
async function getMunicipios(departamentoId) {
    const [rows] = await db_1.pool.query("SELECT id, nombre FROM municipios WHERE departamento_id = ?", [departamentoId]);
    return rows;
}
